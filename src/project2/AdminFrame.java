...
*/
    ...
    public void exportToBusTravel() {
        File file = new File("Bus Travel");

        try {
            FileWriter fw = new FileWriter(file, false);
            BufferedWriter bw = new BufferedWriter(fw);
            int numberOfRow = jTable2.getRowCount();
            System.out.println(jTable2.getRowCount());
            for (int i = 0; i < numberOfRow; i++) {
                for (int j = 0; j < 5; j++) {
                    if (j == 4) {
                        bw.append(jTable2.getValueAt(i, j).toString() + "\n");
